...
本站基于 markdown-online-editor 搭建，您可以点击页面右上角 GitHub 图标查看。

这是一款基于 \`Vue\`、\`Vditor\`，为未来而构建的在线 Markdown 编辑器；轻量且强大：内置粘贴 HTML 自动转换为 Markdown，支持流程图、甘特图、时序图、任务列表，可导出携带样式的图片、PDF、微信公众号特制的 HTML 等等。

---

## 如何使用

**微注**：清空目前这份默认文档，即处于可使用状态。本编辑器另一大优点在于：编辑内容只会在您本地进行保存，不会上传您的数据至服务器，**绝不窥测用户个人隐私，可放心使用**；

默认为 [所见即所得](https://hacpai.com/article/1577370404903) 模式，可通过 \`⌘-⇧-M\`（\`Ctrl-⇧-M\`）进行切换；或通过以下方式：
- 所见即所得：\`⌘-⌥-7\`（\`Ctrl-alt-7\`）
- 即时渲染：\`⌘-⌥-8\`（\`Ctrl-alt-8\`）
- 分屏渲染：\`⌘-⌥-9\`（\`Ctrl-alt-9\`）
